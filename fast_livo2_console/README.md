...
bag 大小并保持实时预览，点击 `DONE / CLOSE` 才返回菜单。
...
后，脚本在现有相机容器内直接执行一次性的 `multi_fast_calib`。

2026-07-21 的正面、偏左、偏右三组联合结果 RMSE 为 `0.0101 m`，结果目录为
`20260721-143813-multi`。外参 `Rcl/Pcl` 写入 FAST-LIVO2 的 `config/mid360.yaml`；
`config/camera_pinhole.yaml` 只保存新相机的内参。相机、镜头或雷达安装位置发生
变化后必须重新标定。
